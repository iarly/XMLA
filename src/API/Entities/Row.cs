...
namespace API.Entities
{
	...
	public partial class Row
	{
		...
		/// 
		/// </summary>
		public Row(object rowData) : this()
		{
			rowData.GetType().GetProperties().ToList().ForEach(prop =>
			{
				var elementInfo = Attribute.GetCustomAttribute(prop, typeof(XmlElementAttribute)) as XmlElementAttribute;
				var propValue = prop.GetValue(rowData, null);
				string elementName = elementInfo.ElementName;

				if (string.IsNullOrEmpty(elementName))
					elementName = prop.Name;

				var newNode = RowNode.CreateNode(elementName, propValue == null ? string.Empty : prop.GetValue(rowData, null).ToString());
				var previousElement = this.RowElements.FirstOrDefault(x => x.Name.Equals(newNode.Name, StringComparison.InvariantCultureIgnoreCase));
				if (previousElement != null)
					this.RowElements.Remove(previousElement);
				this.RowElements.Add(newNode);
			});
		}
		/// <summary>
		/// 
